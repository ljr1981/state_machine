...
feature {NONE} -- Initialization
	...
	make (a_transit: TUPLE [start_id, stop_id: INTEGER; name: STRING]; a_trigger_callback_agent: PROCEDURE [ANY, TUPLE [detachable ANY]]; a_operations: like operations; a_post_transition_operations: like post_transition_operations)
		...
		require
			non_zero: a_transit.start_id > 0 and a_transit.stop_id > 0
			has_operations: a_operations.count > 0
		do
			start := a_transit.start_id
			stop := a_transit.stop_id
			name := a_transit.name
			...
			start_set: start = a_transit.start_id
			stop_set: stop = a_transit.stop_id
...
feature {SM_MACHINE} -- Access
	...
	name: STRING
			-- `name' of Current {SM_TRANSITION} `start' `stop'.
