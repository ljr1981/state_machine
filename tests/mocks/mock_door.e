	...
	initialize_transition_operations (a_machine: SM_MACHINE)
		...
		do
			a_machine.add_transitions (<<
					create {SM_TRANSITION}.make ([1, 2, "Open"], agent open.set, <<agent set_opened>>, <<agent set_fully_opened>>),
					create {SM_TRANSITION}.make ([2, 1, "Close"], agent close.set, <<agent set_closed>>, <<agent set_fully_closed>>)
										>>)
		end
